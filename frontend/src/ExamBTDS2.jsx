...
import questionsData from './preguntas2.json';

function ExamBTDS2({ goToExams }) { // Recibe goToExams como prop
  const [selectedSubject, setSelectedSubject] = useState("");
  const [userAnswers, setUserAnswers] = useState({});
  const [showResults, setShowResults] = useState(false);

  console.log("questionsData:", questionsData); // Verifica los datos

  const subjects = Object.keys(questionsData);

  const handleSubjectChange = (e) => {
    const subject = e.target.value;
    console.log("Selected subject:", subject); // Verifica el tema seleccionado
  ...
  console.log("currentQuestions:", currentQuestions); // Verifica las preguntas actuales
  ...
  const saveResults = () => {
    ...
    const dataToSend = {
      ...
      Semestre: "Segundo Semestre", // Cambia dinámicamente si es necesario
