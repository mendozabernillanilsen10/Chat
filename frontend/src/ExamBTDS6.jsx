...
function ExamBTDS6({ goToExams }) {
  ...
  const ResultadosGrafica = () => {
    const data = [
      { name: "Correctas", value: correctAnswersCount },
      { name: "Incorrectas", value: currentQuestions.length - correctAnswersCount },
    ];
  ...
  return (
    <div className={styles["app-container"]}>
        ...
        <div className={styles["questions-section"]}>
          ...
          {showResults && (
            <div className={styles["results-container"]}>
              ...
              <button
                className={styles["graph-toggle-button"]}
                onClick={() => setShowGraph((prev) => !prev)}
              >
                {showGraph ? "Ocultar estadísticas" : "Ver progreso"}
              </button>
              {showGraph && <ResultadosGrafica />}
            </div>
          )}
        </div>
      )}
      ...
      <div className={styles["exit-section"]}>
        <button className={styles["exit-button"]} onClick={goToExams}>
          Salir del Examen
        </button>
      </div>
    </div>
